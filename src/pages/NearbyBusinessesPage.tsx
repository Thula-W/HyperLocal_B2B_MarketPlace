...
import { useAuth, useUserPlan } from '../contexts/AuthContext';
...
const NearbyBusinessesPage: React.FC = () => {
  ...
  useEffect(() => {
    if (plan === 'premium' && user?.companyDetails?.address) {
      loadNearbyBusinesses();
    } else if (plan === 'free') {
      setBusinesses([]); // Clear businesses for free users
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, loadNearbyBusinesses, plan]);
  ...
  // For free users, dashboard is blurred and overlay is shown
  const isPremium = plan === 'premium';

  return (
    <div className="min-h-screen bg-gray-50 py-8 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={isPremium ? '' : 'pointer-events-none filter blur-sm select-none opacity-60'}>
          {/* Header */}
          <div className="mb-8">
        ...
        {!isPremium && (
          <div className="absolute inset-0 flex items-center justify-center z-20">
            <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-xl p-8 max-w-md mx-4 text-center">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">
                Premium Feature
              </h2>
              <p className="text-gray-600 mb-6">
                Unlock access to nearby businesses and expand your network with our Premium plan.
              </p>
              <button
